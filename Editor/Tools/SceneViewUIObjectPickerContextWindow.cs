...
public class SceneViewUIObjectPickerContextWindow : EditorWindow
{
    ...
    private static readonly List<ICanvasRaycastFilter> raycastFilters =
        new List<ICanvasRaycastFilter>(4);

    private static double lastRightClickTime;
    private static Vector2 lastRightPos;
    private static bool blockSceneViewInput;

    private readonly MethodInfo screenFittedRectGetter = typeof(EditorWindow).Assembly
    ...
    private void ShowContextWindow(List<Entry> results)
    {
        ...
        foreach (string label in uiObjectLabels)
            preferredWidth = Mathf.Max(
                preferredWidth,
                rowGUIStyle.CalcSize(new GUIContent(label)).x
            );
        ...
        position = (Rect)
            screenFittedRectGetter.Invoke(
                null,
                new object[3]
                {
                    new Rect(
                        GUIUtility.GUIToScreenPoint(Event.current.mousePosition)
                            - new Vector2(0f, position.height),
                        position.size
                    ),
                    true,
                    true
    ...
    private void InitializeUIObjectsRecursive(List<Entry> results, int depth, StringBuilder sb)
    {
        foreach (Entry entry in results)
        {
            ...
            uiObjectLabels.Add(
                sb.Append(' ', depth * 4).Append(entry.RectTransform.name).ToString()
            );
    ...
    protected void OnGUI()
    {
        ...
        {
            RectTransform hoveredUIObject =
                (hoveredRowIndex >= 0) ? uiObjects[hoveredRowIndex] : null;
...
#endif
        {
            ...
            switch (ev.type)
            {
                ...
                case EventType.MouseUp:
                {
                    if (
                        ev.button == 1
                        && EditorApplication.timeSinceStartup - lastRightClickTime < 0.2
    ...
    private static void OnSceneViewRightClicked(SceneView sceneView)
    {
        ...
        Vector2 pointerPos = HandleUtility.GUIPointToScreenPixelCoordinate(
        ...
        rootEntry.Children.RemoveAll(
            (canvasEntry) =>
                canvasEntry.Children.Count == 0
                && !canvasEntry.RectTransform.GetComponent<Graphic>()
        );

        // If any results found, show the context window
        if (rootEntry.Children.Count > 0)
            CreateInstance<SceneViewUIObjectPickerContextWindow>()
                .ShowContextWindow(rootEntry.Children);
    }
    ...
    )
    {
        ...
        if (
            RectTransformUtility.RectangleContainsScreenPoint(rectTransform, pointerPos, camera)
            && ShouldCheckRectTransform(rectTransform, pointerPos, camera, ref culledByCanvasGroup)
        )
        {
    ...
    private static bool ShouldCheckRectTransform(
        RectTransform rectTransform,
        Vector2 pointerPos,
        Camera camera,
    ...
    )
    {
        ...
        if (!culledByCanvasGroup)
        {
            ...
            if (
                maskableGraphic != null && !maskableGraphic.maskable && maskableGraphic.color.a > 0f
            )
                return true;
