...
#region VARIABLES
        ...
        private Vector2 _uiPivot = new Vector2(0.5f, 0.5f);
...
#region WINDOW
        ...
        void OnEnable()
        {
            ...
            maxSize = minSize = new Vector2(840, 747);
#else
            maxSize = minSize = new Vector2(840, 751);
#endif
        }
        ...
        private bool BrowsePanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox, GUILayout.Width(411)))
            {
                if (GUILayout.Button("Browse...", _skin.button))
                {
                    ...
                    var psdPath = EditorUtility.OpenFilePanel(
                        "Select Psd file to import...",
                ...
                using (new GUILayout.HorizontalScope())
                {
                    EditorGUILayout.LabelField(
                        System.IO.Path.GetFileName(_psdPath),
        ...
        private void PreviewPanel()
        {
            if (_updatePreview)
            {
                ...
                _resultTexture = GetPreviewTexture(
                    (int)_previewRect.width,
                    (int)_previewRect.height
                );
        ...
        private void ImportVisiblePanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                {
                    GUILayout.Space(4);
                    _importOnlyVisibleLayers = EditorGUILayout.ToggleLeft(
        ...
        {
            var labelMiddleRightStyle = new GUIStyle(GUI.skin.label)
            {
                alignment = TextAnchor.MiddleRight
            };
            var textFieldMiddleRightStyle = new GUIStyle(EditorStyles.textField)
            {
                alignment = TextAnchor.MiddleRight
            };
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    _importWidth = EditorGUILayout.IntField(
                        _importWidth,
                        textFieldMiddleRightStyle,
                        GUILayout.Width(100)
                    );
                    _importWidth = Mathf.RoundToInt(
                        EditorGUILayout.FloatField(
                            (float)_importWidth / _pixelsPerUnit,
                            textFieldMiddleRightStyle,
                            GUILayout.Width(100)
                        ) * _pixelsPerUnit
                    );
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    _importHeight = EditorGUILayout.IntField(
                        _importHeight,
                        textFieldMiddleRightStyle,
                        GUILayout.Width(100)
                    );
                    _importHeight = Mathf.RoundToInt(
                        EditorGUILayout.FloatField(
                            (float)_importHeight / _pixelsPerUnit,
                            textFieldMiddleRightStyle,
                            GUILayout.Width(100)
                        ) * _pixelsPerUnit
                    );
                ...
                using (new GUILayout.HorizontalScope())
                {
                    GUILayout.Label(
                        "Pixels Per Unit: ",
                        labelMiddleRightStyle,
                        GUILayout.Width(204)
                    );
                    _pixelsPerUnit = EditorGUILayout.FloatField(
                        _pixelsPerUnit,
                        textFieldMiddleRightStyle,
                        GUILayout.Width(100)
                    );
        ...
        private readonly string[] _alingmentNames = new string[]
        {
            "Center",
            "Top Left",
            "Top Center",
        ...
        private void PivotPanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                _spriteAlignment = (SpriteAlignment)
                    EditorGUILayout.Popup("Sprite Pivot:", (int)_spriteAlignment, _alingmentNames);
                switch (_spriteAlignment)
                {
                    case SpriteAlignment.Center:
                        _spritePivot = new Vector2(0.5f, 0.5f);
                        break;
                    case SpriteAlignment.TopLeft:
                        _spritePivot = new Vector2(0f, 1f);
                        break;
                    case SpriteAlignment.TopCenter:
                        _spritePivot = new Vector2(0.5f, 1f);
                        break;
                    case SpriteAlignment.TopRight:
                        _spritePivot = new Vector2(1f, 1f);
                        break;
                    case SpriteAlignment.LeftCenter:
                        _spritePivot = new Vector2(0f, 0.5f);
                        break;
                    case SpriteAlignment.RightCenter:
                        _spritePivot = new Vector2(1f, 0.5f);
                        break;
                    case SpriteAlignment.BottomLeft:
                        _spritePivot = new Vector2(0f, 0f);
                        break;
                    case SpriteAlignment.BottomCenter:
                        _spritePivot = new Vector2(0.5f, 0f);
                        break;
                    case SpriteAlignment.BottomRight:
                        _spritePivot = new Vector2(1f, 0f);
                        break;
                    default:
                        break;
                }
                using (new EditorGUI.DisabledGroupScope(_spriteAlignment != SpriteAlignment.Custom))
                {
                    _spritePivot = EditorGUILayout.Vector2Field(string.Empty, _spritePivot);
                }
            }
        }
        ...
        private void AnchorPanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    GUILayout.Space(15);
                    GUILayout.Label("Min:");
                    _anchorMin = EditorGUILayout.Vector2Field(String.Empty, _anchorMin);
                }
                using (new GUILayout.HorizontalScope())
                {
                    GUILayout.Space(15);
                    GUILayout.Label("Max:");
                    _anchorMax = EditorGUILayout.Vector2Field(String.Empty, _anchorMax);
                }
                GUILayout.Space(4);
                using (new GUILayout.HorizontalScope())
                {
                    GUILayout.Label("UI pivot:");
                    _uiPivot = EditorGUILayout.Vector2Field(String.Empty, _uiPivot);
                }
            }
        }
        ...
        private void AtlasPanel()
        {
            using (new GUILayout.HorizontalScope(EditorStyles.helpBox))
            {
                ...
                if (_createAtlas)
                    _atlasMaxSize = EditorGUILayout.IntField("Atlas Max Size: ", _atlasMaxSize);
            }
        }
        ...
        private void OutputFolderPanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    {
                        var outputFolder = EditorUtility.OpenFolderPanel(
                        ...
                        {
                            _outputFolder =
                                outputFolder.Replace(Application.dataPath, "Assets") + "/";
                        }
                        else if (outputFolder != "")
                        {
                            EditorUtility.DisplayDialog(
                                "Output Folder Error",
                                "Output folder must be under Assets folder",
                                "Ok"
                            );
                        }
                    }
        ...
        private void RootObjectPanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    GUILayout.Space(10);
                    _importIntoSelectedObject = EditorGUILayout.ToggleLeft(
                        "Import into selected object",
        ...
        private void PsComponentsPanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                {
                    GUILayout.Space(4);
                    _addPsComponents = EditorGUILayout.ToggleLeft(
                        "Add Ps Components:",
                        _addPsComponents
                    );
                    if (_addPsComponents)
                    {
                        EditorGUIUtility.labelWidth = 110;
                        _blendingShader = (PsGroup.BlendingShaderType)
                            EditorGUILayout.Popup(
                                "Blending Sahder:",
                                (int)_blendingShader,
                                new string[]
                                {
                                    PsGroup.GetShaderTypeName(PsGroup.BlendingShaderType.DEFAULT),
        ...
        {
            var importScale =
                _psdFile.BaseLayer.Rect.width > _psdFile.BaseLayer.Rect.height
            ...
            using (new GUILayout.HorizontalScope())
            {
                ...
                {
                    var textures = PsLayerCreator.CreatePngFiles(
                        _outputFolder,
                        _layerTextures,
                        _psdFile,
                        importScale,
                ...
                if (GUILayout.Button("Create Sprites", _skin.button))
                {
                    if (BlendModeWarning())
                    {
                        ...
                        _layerCreator.CreateGameObjets(_anchorMin, _anchorMax, _uiPivot);
                    }
                }
            ...
            using (new GUILayout.VerticalScope(GUILayout.MaxWidth(826)))
            {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    using (new GUILayout.VerticalScope())
                    {
                        ...
                        PreviewPanel();
                        using (
                            new GUILayout.VerticalScope(EditorStyles.helpBox, GUILayout.Width(411))
                        )
                        {
                            ImportVisiblePanel();
                            SizePanel();
        ...
        private bool ScreenSpaceWarning()
        {
            ...
            return EditorUtility.DisplayDialog(
                "Warning: Render mode will change.",
                "The Fast shader only works with: Screen Space - Camera.\n\n"
                    ...
                    + "Or Would you like to go back and select a different shader?",
                "Continue",
        ...
        private bool BlendModeWarning()
        {
            if (!_addPsComponents)
                return true;
            if (_blendingShader == PsGroup.BlendingShaderType.GRAB_PASS)
                return true;
                ...
                if (layer.VisibleInHierarchy && layer.Visible)
                {
                    switch (_blendingShader)
                    {
                        case PsGroup.BlendingShaderType.DEFAULT:
                            if (
                                layer.BlendModeKey != PsdBlendModeType.NORMAL
                                && layer.BlendModeKey != PsdBlendModeType.PASS_THROUGH
                            )
                            {
                                return EditorUtility.DisplayDialog(
                                    "Warning: Unsupported blend modes.",
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            layer.Visible = EditorGUILayout.Toggle(
                                layer.Visible,
                                _skin.GetStyle(
                                    layer.VisibleInHierarchy
                                        ? "visibilityToggle"
                                        : "invisibleToggle"
                                ),
                    ...
                    )
                    {
                        ...
                        )
                        {
                            if (layer is LayerGroup)
                            {
                                ...
                                GUILayout.Space(14);
                                groupItem.IsOpen = EditorGUILayout.Toggle(
                                    groupItem.IsOpen,
                                    _skin.GetStyle("folderToggle")
                                );
                            }
                            else
                            {
                                ...
                                GUILayout.Space(4);
                                GUILayout.Label(
                                    layerItem.TextureWithBorder,
                                    GUILayout.Width(_hierarchyThumbnailH),
                        ...
                        )
                        {
                            ...
                            GUILayout.Label(
                                ...
                                GUILayout.MaxWidth(186 - indentSize)
                            );
                        }
                        using (
                            new EditorGUILayout.VerticalScope(
                        ...
                        {
                            EditorGUILayout.LabelField(
                                "Blending: " + ((PsdBlendModeType)layer.BlendModeKey).Name,
                                EditorStyles.miniLabel,
                                GUILayout.MaxHeight(15),
                                GUILayout.Width(117)
                            );
                            EditorGUILayout.LabelField(
                                "Opacity: " + (int)((float)layer.Opacity / 2.55f) + "%",
                                EditorStyles.miniLabel,
                                GUILayout.MaxHeight(15),
                                GUILayout.Width(117)
                            );
...
#region LOAD TEXTURES
        ...
        private void OnTextureLoadingComplete(
            Layer layer,
...
#region PREVIEW IMAGE
        private Texture2D GetPreviewTexture(int width, int height)
        {
            ...
            for (int layerIdx = _previewLayers.Count - 1; layerIdx >= 0; --layerIdx)
            {
                ...
                for (int i = 0; i < resultTexturePixels.Length; ++i)
                {
                    resultTexturePixels[i] = GetBlendedPixel(
                        resultTexturePixels[i],
                        sourcePixels[i],
                        previewLayer.Alpha,
        ...
        private static FloatColor SetLuminosity(FloatColor RGBColor, float L)
        {
            ...
            return ClipColor(
                new FloatColor(RGBColor.r + delta, RGBColor.g + delta, RGBColor.b + delta)
            );
        }
        ...
        private static Color GetBlendResult(
            Color backdrop,
            Color source,
            ColorComponent.BlendDelegate blendDelegate
        )
        {
        ...
        internal static Color GetBlendedPixel(
            Color backdrop,
            Color source,
            float layerAlpha,
            PsdBlendModeType blendMode
        )
        {
            ...
            {
                var result = (Color)SetLuminosity(
